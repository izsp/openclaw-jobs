...
function TabBar({ files, activeIndex, onSelect }: TabBarProps) {
  return (
    <div className="flex overflow-x-auto border-b border-zinc-700/50">
      {files.map((file, i) => (
        <button
          key={file.id}
          onClick={() => onSelect(i)}
...
function DownloadButton({ onClick }: DownloadButtonProps) {
  return (
    <button
      onClick={onClick}
      title="Download file"
      className="absolute right-2 top-1 z-10 rounded px-1.5 py-0.5 text-[10px] text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-300"
    >
      &#8615;
    </button>
  );
